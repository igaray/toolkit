...
impl Config {
  pub fn new() -> Config {
    ...
    match fs::File::open(config_path) {
      ...
      Err(reason) => {
        error!("Unable to read configuration file. Loading defaults. Error: {:?}", reason);
        // Build default config.
        let a1 = Agent{ kind: agent::AgentKind::Reactive };
        let a2 = Agent{ kind: agent::AgentKind::Reactive };
        ...
        let c = Config{ debug: true, scenarios: scs };
