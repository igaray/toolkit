...
impl Engine {
    ...
    fn handle_msg_main(&mut self, msg: EngineMessage) {
        match msg {
            ...
            EngineMessage::ReqStart {
                scenario_name,
                world_name,
            } => {
                debug!("Message: MsgStart");
                debug!(
                    "Starting simulation in world {} under {} scenario",
                    world_name, scenario_name
                );

                // Initialize the simulation
                match self.init_sim(scenario_name, world_name) {
                    Ok(_) => {
                        let msg = EngineMessage::ResStart {
                            error: EngineMessageResult::Ok,
                        };
                        self.repl_mailbox.send(msg).unwrap();
                    ...
                    Err(_) => {
                        ...
                        let msg = EngineMessage::ResStart {
                            error: EngineMessageResult::Err,
                        };
                        self.repl_mailbox.send(msg).unwrap();
                    }
                }
            }
            EngineMessage::MsgLoad {
                savefile: _savefile,
            } => {
                        ...
                        match scenario.starting_units {
                            Some(ref units) => {
                                for agent_description in units.iter() {
                                    ...
                                    let agent =
                                        agent::Agent::new(&agent_description.kind, position);
