...
impl Repl {
    ...
    fn state_init(&mut self, command: ReplCommand) {
        ...
        match command {
            ...
            ReplCommand::Start {
                scenario_name,
                world_name,
            } => self.execute_command_start(scenario_name, world_name),
    ...
    fn execute_command_start(&mut self, scenario_name: String, world_name: String) {
        ...
        match self.repl_mailbox.recv().unwrap() {
            engine::EngineMessage::ResStart { error } => match error {
    ...
    fn execute_command_load(&mut self, savefile: String) {
        let msg = engine::EngineMessage::MsgLoad {
    ...
    fn execute_command_save(&mut self, savefile: String) {
        let msg = engine::EngineMessage::MsgSave {
            savefile: savefile.clone(),
        };
        self.engine_mailbox.send(msg).unwrap();
    }
    ...
    fn execute_command_inspect(&mut self, target: String) {
        match target.as_str() {
            "config" => println!("{}", serde_yaml::to_string(&self.config).unwrap()),
            "scenario_config" => println!("{}", serde_yaml::to_string(&self.config).unwrap()),
            &_ => println!("Unknown inspect target."),
        }
    }

    fn execute_command_quit(&mut self) {
        self.engine_mailbox
            .send(engine::EngineMessage::MsgQuit)
            .unwrap();
...
fn parse(input: String) -> Result<ReplCommand, String> {
    ...
    match tokens[0] {
        ...
        "step" => {
            if 1 < tokens.len() {
                match tokens[1].parse::<u8>() {
                    ...
                    Err(_reason) => {
                        return Err("Cannot convert the given value to an integer.".to_string())
                    }
                }
            } else {
        ...
        _ => return Err("Input error: unknown command.".to_string()),
    }
}
