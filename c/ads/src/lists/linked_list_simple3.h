...
void sll_free(simply_linked_list_t *list); /* Destroy a list and free all its
                                              memory. O(n) where n: number of
                                              elements in the list */
sll_node_t *
sll_first(list_t list); /* Extract the element at the beginning of the list. */
...
sll_node_t *
sll_findp(list_t list,
          void *element); /* Find the position of an element in a list. */
unsigned int
sll_findi(list_t list,
          void *element); /* Find the position of an element in a list. */
void *
sll_getp(list_t list,
         sll_node_t *position); /* Retrieve an element at a given position. */
void *
sll_geti(list_t list,
         unsigned int index); /* Retrieve an element at a given position. */
unsigned int sll_is_empty(list_t list); /* Inspect whether the list is empty. */
unsigned int
sll_length(list_t list); /* Inspect the number of elements in the list. */
void sll_insertp(list_t list, void *element, sll_node_t *position);
void sll_inserti(
    list_t list, void *element,
    unsigned int index); /* Insert an element at a given position. */
void sll_insert_first(
    list_t list,
    void *element); /* Insert an element at the beginning of the list. */
void sll_insert_last(
    list_t list, void *element); /* Insert an element at the end of the list. */
void *sll_removep(list_t list,
                  sll_node_t *position); /* Expunge an element at a given
                                            position from the list.*/
void *sll_removei(list_t list, unsigned int index); /* Expunge an element at a
                                                       given position from the
                                                       list.*/
void list_concat(list_t *list1, list_t *list2);     /* Concatenate two lists. */
void list_sort(list_t *list)                        /* Sort a list in-place. */
    void sll_print(list_t list, char *string,
                   unsigned int *size); /* Prints out the elements in a list,
                                           mostly for debugging purposes. */

/*
x in s                              member(list, element)
x not in s
s + t                               concatenate(list, list, list) copy
semantics?
s[i]
...
those of t
del s[i:j:k]                        removes the elements of s[i:j:k] from the
list
...
/**************************************************************************************************/
unsigned int simply_linked_list_findi(list_t list, void *element) { return 0; }
...
/**************************************************************************************************/
void simply_linked_list_insertp(list_t list, void *element,
                                sll_node_t *position) {
  // TODO
}

/**************************************************************************************************/
void simply_linked_list_inserti(list_t list, void *element,
                                unsigned int index) {
  // TODO
}
...
void simply_linked_list_insert_last(list_t list, void *element) {
  sll_node_t *position = *list, node = malloc(sizeof(node_t));
